...
class Preprocessor():
    ...
    def get_save_path(self):
        self.save_path = os.path.join(os.getcwd(), 'data', 'preprocessed', self.sim, f'expert_data_{self.num}')
    ...
    def get_s_AND_s_next(self, dict_result):
        patience = int(self.delta_t / self.cycle_time)
        ...
        return deepcopy(data[:len(data)-patience,:]), deepcopy(data[patience:,:])
...
class SeqPreprocessor():
    ...
    def get_s_AND_s_next(self, dict_result):
        ...
        return deepcopy(data[:len(data)-patience,:]), deepcopy(np.reshape(data[patience:,-1,:], (len(data)-patience, data.shape[-1])))
